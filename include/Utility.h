...
void OutputBinA(UCHAR *szBin, UINT nLength, CHAR *szFileName = nullptr, CHAR *szTitle = nullptr);
void OutputBinW(UCHAR *szBin, UINT nLength, WCHAR *szFileName = nullptr, WCHAR *szTitle = nullptr);


// 取指定参数的一个字节
#define  _GetByte(p,n)  ((byte*)&p)[n]
#define  _GetInt16(p)	MAKEWORD(((byte *)&p)[1],((byte *)&p)[0])
#define  _GetInt32(p)	MAKELONG(GetInt16(((byte *)&p)[2]),GetInt16(p))
#define  _GetInt64(p)	MAKEUINT64(GetInt32(((byte *)&p)[4]),GetInt32(p))
...
void GetLastErrorMessageA(IN LPCSTR szCaption, OUT LPSTR szErrorText, IN DWORD dwErrorCode = 0);
void GetLastErrorMessageW(IN LPCWSTR szCaption, OUT LPWSTR szErrorText, IN DWORD dwErrorCode = 0);
