...
void GetLastErrorMessageW(IN LPCWSTR szCaption,OUT LPWSTR szText,IN DWORD dwErrorCode)
{
	LPVOID lpMsgBuf = NULL;
	__try
	{
...
{ 
	...
	{ 
		...
		if (!PathFileExistsA(szTempDir))
		{
			...
			TraceMsg(_T("%s Create Succeed.\r\n"),__FUNCTIONW__,_UnicodeString(szTempDir,CP_ACP));
		}
		else
		{
