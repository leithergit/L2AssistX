...
// 		LeaveCriticalSection(&m_csRecvList);
	...
	int AsyncConnect(const CHAR *szServer,int nPort,int nLocalPort = -1)
	{	
		DWORD dwErrorCode = 0;
...
// 			}
	...
	int Recv()
	{
		...
		//TraceMsgA("%s nBufferLength = %d.\n", __FUNCTION__, nBufferLength);
	...
	int Send(byte *szBuffer,int nLeng)
	{
		...
		if (nSendNum != 0 && nSendNum != SOCKET_ERROR)
		{
			if (m_bEnableStreamStat)
			{
				EnterCriticalSection(&m_csSendList);
...
public:
	...
	void CheckFreeClient(LPThreadInfo pThreadInfo)
	{
		...
		for (int nIndex = pThreadInfo->dwEventCount - 1; nIndex >= 0;nIndex --)
		{
			...
			if (TimeSpanEx(pClient->tLastAction) >= m_fMaxFreeTime)
			{
#ifdef _DEBUG
